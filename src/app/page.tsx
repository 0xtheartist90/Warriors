...
const partners = [
    ...
    { name: 'Partner', src: `${WIX}/c5b4b1_0d55e1f3b8cd4eaaa0447b2d40bda8df~mv2.png` },
    { name: 'Partner', src: `${WIX}/c5b4b1_6b5b231a509a4ed28133e6822575b300~mv2.png` },
    { name: 'Partner', src: `${WIX}/c5b4b1_d57d3bec4a304d408d1c5c720c58cc44~mv2.png` },
    { name: 'Partner', src: `${WIX}/c5b4b1_dc096df71b9a4cd58dd869d4cd986d4f~mv2.png` },
    { name: 'Partnerclub', src: `${WIX}/3d8cdb_0a6050ac051f4e78a63e61afe0107d9e~mv2.jpg` },
    ...
    { name: 'Partner', src: `${WIX}/c5b4b1_06d6d3bf68d44d109dc0831ce2c64bad~mv2.jpeg` },
...
const GoldButton = ({ href, children, external }: { href: string; children: React.ReactNode; external?: boolean }) => (
    <Link
        ...
        className='display bg-gold text-navy hover:bg-gold-dark inline-flex items-center gap-2 rounded-full px-7 py-3 text-sm tracking-widest transition-colors'>
        {children}
...
const GhostButton = ({ href, children, external }: { href: string; children: React.ReactNode; external?: boolean }) => (
    <Link
        href={href}
        {...(external ? { target: '_blank', rel: 'noreferrer noopener' } : {})}
        className='display hover:border-gold hover:text-gold inline-flex items-center gap-2 rounded-full border border-white/30 px-7 py-3 text-sm tracking-widest text-white transition-colors'>
        {children}
    </Link>
);
...
const Page = () => {
    return (
        <main className='overflow-x-clip'>
            {/* ---------------- Header ---------------- */}
            <header className='border-gold/20 bg-navy/85 fixed inset-x-0 top-0 z-50 border-b backdrop-blur'>
                <div className='mx-auto flex h-20 max-w-[1280px] items-center justify-between px-6'>
                    ...
                    <nav className='hidden items-center gap-8 lg:flex'>
                        {navItems.map((item) => (
                            <Link
                                key={item.label}
                                href={item.href}
                                className='display hover:text-gold text-sm tracking-widest text-white/80 transition-colors'>
                                {item.label}
                            </Link>
                        ))}
                    </nav>
                    <Link
                        ...
                        className='display bg-gold text-navy hover:bg-gold-dark rounded-full px-6 py-2.5 text-sm tracking-widest transition-colors'>
                        Word lid
                    </Link>
                </div>
            </header>
            ...
            <section className='relative flex min-h-svh items-center overflow-hidden pt-20'>
                ...
                <div className='from-navy absolute inset-x-0 bottom-0 h-40 bg-gradient-to-t to-transparent' />
                {/* gouden decoratieve blokken */}
                <div aria-hidden className='bg-gold absolute top-32 right-10 hidden size-24 lg:block' />
                <div aria-hidden className='border-gold absolute right-24 bottom-24 hidden size-40 border-4 lg:block' />

                <div className='relative z-10 mx-auto w-full max-w-[1280px] px-6 py-24'>
                    <p className='eyebrow'>Basketbalclub Amsterdam Zuidoost</p>
                    <h1 className='display mt-6 max-w-3xl text-[clamp(3rem,8vw,6rem)] text-white'>
                        Work hard <span className='text-gold'>play hard</span>
                    </h1>
            ...
            <section className='border-gold/15 border-y'>
                <div className='mx-auto grid max-w-[1280px] grid-cols-2 gap-8 px-6 py-12 lg:grid-cols-4'>
                    {stats.map((stat) => (
                        <div key={stat.label} className='flex items-baseline gap-3'>
                            <span className='display text-gold text-5xl'>{stat.value}</span>
                            <span className='display max-w-28 text-sm leading-tight text-white/70'>{stat.label}</span>
                        </div>
                    ))}
                </div>
            </section>
                            ...
                            {['Veilige omgeving', 'Sportclub voor iedereen', 'Samenwerken', 'Plezier voorop'].map(
                                (item) => (
                                    <li key={item} className='flex items-center gap-3'>
                                        <span className='bg-gold text-navy flex size-6 shrink-0 items-center justify-center rounded-full'>
                                            <Check className='size-4' strokeWidth={3} />
                                        </span>
                                        <span className='display text-sm tracking-wide text-white'>{item}</span>
                                    </li>
                                )
                            )}
                        </ul>
                ...
                <div className='mx-auto max-w-[1280px] px-6 py-24'>
                    ...
                    <div className='mt-14 grid gap-6 md:grid-cols-3'>
                        {programmas.map((programma, index) => (
                            <article key={programma.title} className='bg-navy-card group flex flex-col'>
                                <div className='relative aspect-[16/10] overflow-hidden'>
                                    ...
                                    />
                                    <div aria-hidden className='bg-gold absolute top-0 left-0 h-1.5 w-full' />
                                </div>
                                <div className='flex flex-1 flex-col p-7'>
                                    <span className='display text-gold text-sm tracking-widest'>
                                        {String(index + 1).padStart(2, '0')}
                                    </span>
                                    <h3 className='display mt-2 text-2xl text-white'>{programma.title}</h3>
                                    ...
                                    <Link
                                        ...
                                        target='_blank'
                                        rel='noreferrer noopener'
                                        className='display text-gold mt-6 inline-flex items-center gap-2 text-sm tracking-widest hover:underline'>
                                        Meer lezen <ArrowRight className='size-4' />
                                    </Link>
                                </div>
                            </article>
                        ))}
                ...
                <div className='mx-auto grid max-w-[1280px] items-center gap-14 px-6 py-24 lg:grid-cols-2'>
                    <div>
                        ...
                        <p className='mt-6 leading-relaxed text-white/70'>
                            ...
                            veroverd en promoveert nu naar de Eredivisie! 🏆 Dit is de opbrengst van focus, discipline
                            en doorzettingsvermogen. Voor het seizoen 2026-2027 willen we de landelijke top bestormen
                            met twee teams: de U12 en onze debuut U14 in de Eredivisie. We verwelkomen alle jongens en
                            meisjes vanaf het geboortejaar 2016.
                        </p>

                        <div className='border-gold/25 mt-8 border p-6'>
            ...
            <section id='contributie' className='bg-navy-light scroll-mt-24'>
                <div className='mx-auto max-w-[1280px] px-6 py-24'>
                    <div className='grid gap-14 lg:grid-cols-2'>
                        <div>
                            ...
                            <ul className='mt-8 space-y-4'>
                                <li className='flex items-start gap-3'>
                                    <span className='bg-gold text-navy mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full'>
                                        <Check className='size-4' strokeWidth={3} />
                                    </span>
                                    <p className='text-sm text-white/75'>
                                        <strong className='text-white'>In één keer betalen</strong> — geen extra kosten
                                    </p>
                                </li>
                                <li className='flex items-start gap-3'>
                                    <span className='bg-gold text-navy mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full'>
                        ...
                        <div className='bg-navy-card border-gold/25 self-start border-t-4 p-8'>
                            ...
                            <a
                                ...
                                rel='noreferrer noopener'
                                className='display text-gold mt-6 inline-flex items-center gap-2 text-sm tracking-widest hover:underline'>
                                Meer informatie <ArrowRight className='size-4' />
                            </a>
                            <div className='border-gold/15 mt-8 border-t pt-6'>
                                <p className='display text-sm tracking-widest text-white/50'>#WijWerkenMetNIKKI</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <section id='partners' className='scroll-mt-24'>
                <div className='mx-auto max-w-[1280px] px-6 py-24'>
                    ...
                    <p className='mt-5 max-w-xl leading-relaxed text-white/70'>
                        Onze organisatie werkt veel samen met de volgende partijen om zo elkaar te ondersteunen, om een
                        betere wijk te creëren.
                    </p>
                    <div className='mt-12 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6'>
                        {partners.map((partner, index) => (
                            <div
                                key={`${partner.name}-${index}`}
                                className='flex aspect-[3/2] items-center justify-center bg-white p-5'>
                                <Image
                                    src={partner.src}
                                    alt={partner.name}
                                    width={200}
            ...
            <section className='relative overflow-hidden'>
                ...
                <div className='relative z-10 mx-auto flex max-w-[1280px] flex-col items-center px-6 py-28 text-center'>
                    ...
                    <p className='mt-5 max-w-xl text-white/75'>
                        Meld je aan voor een proefles en ervaar zelf hoe leuk basketbal bij de Amsterdam Warriors is.
                    </p>
                    <div className='mt-10'>
                        <GoldButton href={links.proefles} external>
                            Proefles aanvragen
            ...
            {/* ---------------- Footer ---------------- */}
            <footer id='contact' className='border-gold/20 scroll-mt-24 border-t'>
                <div className='mx-auto grid max-w-[1280px] gap-12 px-6 py-16 md:grid-cols-2 lg:grid-cols-4'>
                    <div>
                        ...
                        <ul className='mt-6 space-y-3 text-sm text-white/70'>
                            <li className='flex items-center gap-3'>
                                ...
                                <a className='hover:text-gold' href='mailto:info@amsterdamwarriors.com'>
                                    info@amsterdamwarriors.com
                                </a>
                            </li>
                            <li className='flex items-center gap-3'>
                                <Phone className='text-gold size-4 shrink-0' />
                                <a className='hover:text-gold' href='tel:+31611210422'>
                                    +31 6 11 21 04 22
                                </a>
                            </li>
                            <li className='flex items-center gap-3'>
                    ...
                    <div>
                        <h3 className='display text-gold text-sm tracking-widest'>Aanbod</h3>
                        <ul className='mt-5 space-y-3 text-sm text-white/70'>
                            {['Basketbalschool', 'Regio', 'Excellence', 'Scholentoernooi 2026'].map((item) => (
                                <li key={item}>
                                    <Link className='hover:text-gold' href='#aanbod'>
                                        {item}
                                    </Link>
                                </li>
                            ))}
                    ...
                    <div>
                        <h3 className='display text-gold text-sm tracking-widest'>Nuttige links</h3>
                        <ul className='mt-5 space-y-3 text-sm text-white/70'>
                            <li>
                                <a
                        ...
                        <div className='mt-6 text-sm text-white/60'>
                            <p className='display text-xs tracking-widest text-white/50'>Vertrouwenspersoon</p>
                            <p className='mt-2'>
                                Wilt u iets melden?{' '}
                    ...
                    <div>
                        <h3 className='display text-gold text-sm tracking-widest'>Volg ons</h3>
                        <div className='mt-5 flex gap-3'>
                            {socials.map(({ label, href, Icon }) => (
                                <a
                                    ...
                                    rel='noreferrer noopener'
                                    aria-label={label}
                                    className='border-gold/30 text-gold hover:bg-gold hover:text-navy flex size-10 items-center justify-center rounded-full border transition-colors'>
                ...
                <div className='border-gold/15 border-t'>
                    <p className='mx-auto max-w-[1280px] px-6 py-6 text-center text-xs text-white/50'>
